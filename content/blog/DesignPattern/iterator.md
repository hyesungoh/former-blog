...
본 게시물은 `Java 언어로 배우는 디자인 패턴 입문 - Yuki Hiroshi 저`을 기반으로 공부한 것을 정리하며 Typescript로 재작성해본 내용입니다.
...
위 코드는 `i`를 하나씩 증가시키며 배열 arr의 요소 전체를 처음부터 차례대로 검색하게 된다. 여기에서 사용되고 있는 변수 `i`의 기능을 추상화해서 일반화한 것을 디자인 패턴에서는 `Iterator` 패턴이라고 한다.
...
> Aggregate는 '모으다', '모이다', '집합'의 이미로 `집합체`를 나타낸다.
